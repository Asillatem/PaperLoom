import { ChevronDown, ChevronUp, X, Inbox, Trash2, ArrowRight, GripVertical } from 'lucide-react';
...
export function StagingPopup() {
  ...
  const handleAddToCanvas = (itemId: string) => {
    moveToCanvas(itemId);
  };
  ...
  return (
    <div className="fixed bottom-0 left-1/2 -translate-x-1/2 z-40 w-full max-w-2xl">
      {/* Toggle Tab */}
      <button
            ...
            {stagedItems.map((item) => (
              ...
              >
                ...
                <div className="flex items-center justify-end gap-1 px-2 py-1.5 border-t border-neutral-200 bg-white opacity-0 group-hover:opacity-100 transition-opacity">
                  <button
                    onClick={() => handleAddToCanvas(item.id)}
